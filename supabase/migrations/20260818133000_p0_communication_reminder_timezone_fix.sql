-- P0 correction: preserve the repository's existing appointment time semantics.
-- The clinics table has no timezone column, so do not introduce a new timezone policy in P0.
...
BEGIN
  ...
  -- Separate appointment call reminder. Keep the existing date/time semantics.
