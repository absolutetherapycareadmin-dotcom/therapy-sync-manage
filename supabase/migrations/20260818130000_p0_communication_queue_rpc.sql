-- P0 queue transition helpers keep the new escalation table behind database functions
-- so the existing generated Supabase client types do not need to be regenerated mid-step.
...
AS $$
  SELECT CASE
    ...
    ELSE jsonb_build_object('status', ce.status, 'current_stage', ce.current_stage)
  END
...
DECLARE
  ...
  call_at timestamptz;
BEGIN
  SELECT * INTO e FROM public.communication_escalations WHERE id = p_escalation_id FOR UPDATE;
  ...
  IF s.id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'sms_queue_missing');
  END IF;
  ...
  WHERE communication_event_id = e.id AND call_type = 'escalation' AND recipient_role = 'parent'
  LIMIT 1;
...
BEGIN
  ...
  WHERE id = p_escalation_id AND status = 'waiting_call';
  RETURN FOUND;
END;
$$;
