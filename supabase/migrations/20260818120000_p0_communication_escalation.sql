-- P0 communication escalation foundation
-- Master flow: Appointment Event -> WhatsApp -> configurable wait -> SMS -> configurable wait -> Call -> STOP.
-- This migration preserves the existing provider queues and free WhatsApp/manual-open workflow.
...
-- A message belongs to a specific appointment communication event. This permits a
-- rescheduled appointment to have a new valid workflow without breaking the old log.
...
-- Create one authoritative workflow for an appointment event.
...
BEGIN
  ...
  -- The fallback is scheduled, not sent. A later worker must still verify that
  -- this escalation is active before handing the SMS to the Android device.
  ...
  -- Separate appointment call reminder remains independent of escalation.
...
-- Appointment insert starts exactly one workflow. Duplicate event processing is
-- rejected by the unique (appointment_id,event_key) key.
...
-- Cancellation and reschedule protection. A reschedule supersedes the old event
-- and creates a new workflow for the new appointment date/time.
...
-- Parent action is authoritative: any response cancels all remaining automatic
-- SMS/call escalation for that appointment. No status-update SMS is generated.
